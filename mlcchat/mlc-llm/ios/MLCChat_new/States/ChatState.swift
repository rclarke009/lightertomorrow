...
@MainActor
final class ChatState: ObservableObject, Sendable {
    ...
    func requestGenerate(prompt: String) {
        ...
        Task {
            ...
            if (finishReasonLength) {
                let windowSize = historyMessages.count
                assert(windowSize % 2 == 0)
                let removeEnd = ((windowSize + 3) / 4) * 2
                historyMessages.removeSubrange(0..<removeEnd)
            }
